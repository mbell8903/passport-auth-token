...
Strategy.prototype.authenticate = function (req, options) {
	...
	for (i = 0, len = this._headerFields.length; !token && i < len; i++) {
		token = lookup(req.headers, this._headerFields[i], lookupOptions);
	}

	for (i = 0, len = this._tokenFields.length; !token && i < len; i++) {
		token = lookup(req.body, this._tokenFields[i], lookupOptions) ||
			lookup(req.query, this._tokenFields[i], lookupOptions);

		// Route parameters are a fallback and must not replace a body or query token.
		if (!token && options.params) {
			token = lookup(req.params, this._tokenFields[i], lookupOptions);
		}
	}
