...
- The npm trusted publisher must reference
  `mbell8903/passport-auth-token` and the `publish.yml` workflow, with
...
```sh
npm view passport-auth-token@next version dist.shasum dist.integrity gitHead --json
```

Publish the matching GitHub Release, then promote the reviewed version:

```sh
npm dist-tag add passport-auth-token@<version> latest
npm view passport-auth-token dist-tags --json
```

Both `latest` and `next` should then resolve to the released version.
...
```sh
npm dist-tag add passport-auth-token@<previous-version> latest
```
